...
const translations = {
  ru: {
    mainMenuPrompt: '✨ Добро пожаловать! Выберите действие ниже и давайте начнём 🚀',
    complaintButton: '🆘 Оставить жалобу',
    settingsButton: '⚙️ Центр настроек',
    complaintChooseLine: '📞 На какую линию пришли жалобы? Выберите нужную:',
    complaintLineChosen: ({ lineTitle, lineId }) =>
      [
        '🔎 Линия найдена! ✨',
        `📞 Линия: ${lineTitle || lineId}`,
        '',
        '📝 Опишите ситуацию одним сообщением — чем подробнее, тем быстрее ответ 👇',
      ].join('\n'),
    complaintSent: '✅ Готово! Жалоба улетела администратору. Спасибо за внимание 🙏',
    complaintError: '⚠️ Упс, не получилось отправить жалобу. Напишите администратору, пожалуйста.',
    notActive: '⏳ Ваша заявка пока на проверке. Немного терпения — скоро вернёмся с ответом!',
    notLinked: '🔗 Пока что вы не привязаны ни к одной линии. Свяжитесь с администратором, чтобы исправить это.',
    banned: '⛔️ Доступ закрыт. Пожалуйста, обратитесь к администратору за подробностями.',
    pendingApplied: '📝 Заявка уже у модератора. Мы сообщим, как только появится решение 💬',
    alreadyPending: '📝 Заявка на рассмотрении. Немного подождите — всё под контролем!',
    languagePrompt: '🌐 Выберите язык интерфейса / Choose your interface language:',
    languageReminder:
      '🌐 Пожалуйста, нажмите на подходящую кнопку ниже.\n🌍 Please choose a language using the buttons below.',
    languageConfirmed: ({ languageName }) => `✅ Готово! Интерфейс теперь на языке: ${languageName}.`,
    stopWork: ({ until, message }) =>
      `🚧 ${message}${until ? `\n🗓 Доступ откроется после: ${until}` : ''}`,
    adminPanel: '📋 Панель администратора. Выберите, что настроим?',
    adminUseMenu: '🛠 Используйте команду /admin, чтобы открыть панель управления.',
    noAccessLine: '🚫 У вас пока нет доступа к этой линии. Проверьте права или напишите администратору.',
    lineNotConfigured: '⚠️ Для этой линии ещё не настроена группа логов, поэтому жалоба не отправлена.',
    lineMissing: '❗️ Не нашли такую линию. Сообщите администратору, чтобы он проверил настройки.',
    muteActive: ({ until }) => `🔇 Сейчас вы не можете отправлять сообщения. Ограничение действует до: ${until}.`,
    muteActiveComplaints: ({ until }) =>
      `🔇 Жалобы временно недоступны. Ограничение действует до: ${until}.`,
    applicationDeclinedUser:
      '❌ К сожалению, заявку отклонили. Напишите администратору, чтобы узнать подробности.',
    applicationApprovedUser: ({ lineTitle, lineId }) =>
      `🎉 Отличные новости! Вы привязаны к линии ${lineTitle || lineId}.`,
    applicationApprovedAdmin: ({ lineTitle, lineId }) =>
      `✅ Линия ${lineTitle || lineId} пополнена новым участником.`,
    languageSelectionSent: '🌐 Напоминание выбрать язык отправлено пользователю.',
    selectLineManualHint: '🔢 Укажите ID линии для пользователя (пример: 101).',
    linesListEmpty: '📭 Линии пока не созданы. Создайте первую, чтобы начать.',
    usersListEmpty: '📭 Пользователей ещё нет. Самое время пригласить коллег!',
    adminUsersStatusUpdated: '✅ Готово! Статус пользователя обновлён.',
    adminUsersStatusUnchanged: 'ℹ️ Этот статус уже активен — изменений не потребовалось.',
    adminUsersMuteUpdated: ({ hours }) => `🔇 Ограничение установлено на ${hours} ч.`,
    adminUsersMuteCleared: '🔊 Ограничение снято. Пользователь снова в строю!',
    adminUsersMuteAlreadyCleared: 'ℹ️ Ограничений не найдено — всё уже было снято.',
    adminUsersNotFound: 'Пользователь не найден. Проверьте ID и попробуйте снова.',
    stopWorkActivated: '🚧 Режим стоп-ворка включён. Берём паузу.',
    stopWorkDisabled: '✅ Стоп-ворк отключён. Можно продолжать работу.',
    muteRemoved: ({ userId }) => `🔊 Ограничение для пользователя ${userId} снято.`,
    muted: ({ userId, hours }) => `🔇 Пользователь ${userId} получил ограничение на ${hours} ч.`,
    bannedUser: ({ userId }) => `⛔️ Пользователь ${userId} заблокирован.`,
    adminSettingsTitle: '⚙️ Настройки администратора. Чем займёмся?',
    adminSettingsStopWorkMessageButton: '✏️ Настроить сообщение стоп-ворка',
    adminSettingsShowConfigButton: '📄 Показать текущие настройки',
    adminSettingsStopWorkMessagePrompt:
      '💬 Пришлите новый текст сообщения стоп-ворка по умолчанию. Чтобы вернуть стандарт, отправьте символ "-".',
    adminSettingsStopWorkMessageUpdated:
      '✅ Готово! Сообщение стоп-ворка обновлено.',
    adminSettingsStopWorkMessageCurrent: ({ message }) =>
      `ℹ️ Сейчас установлено сообщение: ${message || '—'}`,
    adminSettingsConfig: ({ stopWorkActive, stopWorkUntil, stopWorkMessage, defaultMessage }) =>
      [
        '⚙️ Сводка настроек:',
        `🚧 Стоп-ворк: ${stopWorkActive ? 'активен' : 'выключен'}`,
        `🗓 Действует до: ${stopWorkUntil || '—'}`,
        `💬 Сообщение стоп-ворка: ${stopWorkMessage || '—'}`,
        `💬 Сообщение по умолчанию: ${defaultMessage || '—'}`,
      ].join('\n'),
    attachedUser: ({ userLabel, lineTitle, lineId }) =>
      `🔗 ${userLabel} теперь работает на линии ${lineTitle || lineId}.`,
    detachedUser: ({ userId, lineId }) => `✂️ Пользователь ${userId} больше не привязан к линии ${lineId}.`,
    lineGroupSet: ({ lineTitle, groupId }) =>
      `📡 Для линии ${lineTitle} назначен чат ${groupId}.`,
    lineCreated: ({ lineTitle, lineId }) => `🆕 Линия ${lineTitle || lineId} создана и готова к работе!`,
    ...
    complaintLogStatusUpdated: '✅ Статус жалобы обновлён. Спасибо за оперативность!',
    complaintLogStatusAlreadySet: 'ℹ️ Статус уже был установлен ранее.',
    complaintLogNoAccess: '🚫 Недостаточно прав для этого действия.',
    complaintPrompt: '📞 Выберите линию, где нужна помощь:',
    backButton: '⬅️ Шаг назад',
    settingsPrompt: '⚙️ Центр настроек. Что хотите изменить?',
    settingsChangeLanguageOption: '🌐 Сменить язык интерфейса',
    settingsInstructionsOption: '📘 Как оставить жалобу',
    settingsInstructions:
      [
        'ℹ️ Инструкция по жалобе:',
        '1️⃣ Нажмите «🆘 Оставить жалобу».',
        '2️⃣ Выберите линию и, если требуется, конкретный номер.',
        '3️⃣ Опишите проблему одним сообщением — команда поддержки моментально её увидит.',
        '',
        'Совет: в настройках всегда можно поменять язык интерфейса.',
      ].join('\n'),
    complaintCancelButton: '❌ Отмена',
    complaintCancelled: '✅ Жалоба отменена. Возвращаемся в главное меню.',
    coldButton: '❄️ Холодные аккаунты',
    coldInstructions:
      'Добавьте сюда холодный аккаунт, и он сможет отправлять жалобы без вашего участия.\nДля него будут доступны только выбор языка и кнопка жалобы.',
    coldNoProfiles: '❄️ Холодки ещё не подключены. Попросите администратора помочь.',
    coldListTitle: '📋 Подключённые холодные аккаунты:',
    coldProfileStatusPending: '⏳ ждёт подключения',
    coldProfileStatusActive: '✅ подключена',
    coldSetupTitle: '❄️ Мастер настройки холодки.',
    ...
    coldSetupChooseLine: 'Выберите линию для этой холодки:',
    coldSetupNoLines: 'Нет доступных линий, чтобы подключить холодку.',
    ...
    coldSetupSaved: ({ username, lineTitle, lineId, sip }) =>
      [
        '✅ Холодка успешно сохранена.',
        `👤 Username: ${username}`,
        `📞 Линия: ${lineTitle || lineId}`,
        `📟 SIP: ${sip}`,
        '🚀 Осталось запустить бота с этого аккаунта, чтобы завершить подключение.',
      ].join('\n'),
    coldSetupUsernameInUse: 'Этот username уже закреплён за другой холодкой.',
    coldSetupCancelled: '❌ Настройка холодки остановлена.',
    ...
    complaintSipReminder:
      '📟 Пожалуйста, выберите номер с помощью кнопок ниже.',
    complaintSipChosen: ({ sip, lineTitle, lineId }) =>
      [
        '🎉 Отлично! Номер выбран.',
        `📟 ${sip}`,
        `📞 Линия: ${lineTitle || lineId}`,
        '',
        '📝 Расскажите, что случилось, одним сообщением — чем подробнее, тем быстрее ответ 👇',
      ].join('\n'),
    ...
    pendingApplicationsList: ({ items }) => `📥 Заявки в очереди\n${items.join('\n')}`,
    pendingApplicationsEmpty: '✨ Активных заявок нет — можно выдохнуть.',
    userListFooter: ({ count }) => `\n... и ещё ${count}`,
    stats: ({ totalUsers, activeUsers, bannedUsers, totalLines, pending }) =>
      [
        `👥 Всего пользователей: ${totalUsers}`,
        `🟢 Активно сейчас: ${activeUsers}`,
        `⛔️ В блокировке: ${bannedUsers}`,
        `📞 Количество линий: ${totalLines}`,
    ...
    sipStatsEmpty: '📭 Пока нет жалоб с SIP-номерами — отличная новость!',
    stopWorkStatus: ({ active, until, message }) =>
      `🚧 Стоп-ворк: ${active ? 'активен' : 'выключен'}${until ? `\n🗓 До: ${until}` : ''}${
        message ? `\n💬 Сообщение: ${message}` : ''
      }`,
    waitingForLineIdFormat: 'ℹ️ Укажите ID линии. Пример формата: 101;Support.',
    attachUserFormat: 'ℹ️ Отправьте в формате: userId;lineId.',
    setGroupFormat: 'ℹ️ Укажите lineId;chatId. Можно просто переслать сообщение из нужной группы.',
    banPrompt: '🔢 Пришлите userId пользователя, которого нужно забанить.',
    mutePrompt: '🔢 Пришлите userId и часы ограничения через точку с запятой (пример: 12345;2).',
    unmutePrompt: '🔢 Пришлите userId;0, чтобы снять ограничение.',
    stopWorkPrompt:
      '🕒 Пришлите дату и текст: YYYY-MM-DD HH:MM;Сообщение. Можно оставить дату пустой.',
    ...
    genericError: '⚠️ Что-то пошло не так. Попробуйте ещё раз немного позже.',
    menuReminder: '🔁 Используйте кнопки меню, чтобы не потеряться.',
  },
  en: {
